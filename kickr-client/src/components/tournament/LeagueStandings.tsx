...
export const LeagueStandings: React.FC<LeagueStandingsProps> = ({ standingsJson, maxEntries = 10 }) => {
    if (!standingsJson) {
        return (
            <div className="py-32 flex flex-col items-center justify-center border border-white/5 bg-black/[0.02] rounded-sm">
    ...
    return (
        <div className="overflow-x-auto rounded-sm border border-white/5 bg-black/[0.02]">
            <table className="w-full text-left border-collapse">
                ...
                <tbody>
                    {displayedStandings.map((entry) => (
                        <tr
                            key={entry.team.id}
                            className="border-b border-white/5 hover:bg-black/[0.02] transition-colors"
                        >
                            <td className="py-2 md:py-3 px-2 md:px-4">
                                <span className="text-[10px] md:text-xs font-black italic text-main/80">
                                    {entry.rank}
                                </span>
                            </td>
                            <td className="py-2 md:py-3 px-2 md:px-4">
