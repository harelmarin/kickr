    ...
    return (
        <section className="bg-black/[0.02] border border-white/5 p-8 rounded-sm">
            <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6">
                ...
                <span className="text-[8px] font-mono text-main/20 uppercase tracking-widest">Community</span>
            </div>
            <div className="space-y-6">
                {isLoading ? (
                        ...
                        <div key={review.id} className="relative group">
                            ...
                            <div className="flex items-center gap-2 mb-2 relative z-10 pointer-events-none">
                                <Link to={`/user/${review.user?.id}`} className="w-4 h-4 rounded-sm overflow-hidden bg-black/10 border border-white/5 pointer-events-auto hover:border-kickr/40 transition-colors">
                                    {review.user.avatarUrl ? <img src={review.user.avatarUrl} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-black/10 flex items-center justify-center text-[7px] font-black italic text-kickr">{review.user.name[0]}</div>}
                                </Link>
                                <Link to={`/user/${review.user?.id}`} className="text-[9px] font-black text-main/40 uppercase tracking-widest hover:text-main transition-colors italic pointer-events-auto">
                            ...
                            <div className="bg-kickr-bg-primary/20 border border-white/5 p-3 rounded-sm group-hover:border-kickr/20 transition-all relative z-10 pointer-events-none">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <span className="text-[9px] font-black text-main/80 uppercase italic tracking-tighter truncate leading-none group-hover:text-kickr transition-colors">
                                        {review.match.homeTeam} VS {review.match.awayTeam}
                                    </span>
                                </div>
                                {review.comment && (
                                    <p className="text-[10px] text-main/40 line-clamp-2 italic leading-relaxed font-medium">
                                        "{review.comment}"
                                    </p>
                                )}
                            </div>
                    ...
                    <p className="text-[10px] text-main/20 italic">No reviews yet.</p>
                )}
            </div>
        </section>
    );
