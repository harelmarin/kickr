      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
        {matches.map((match) => (
          <Link
            key={match.id}
            to={`/matches/${match.id}`}
            className="group relative bg-white/[0.02] border border-white/5 hover:border-kickr/40 hover:bg-white/[0.05] transition-all rounded-sm overflow-hidden flex flex-col"
          >
            {/* Header: Competition */}
            <div className="flex items-center justify-center px-2 md:px-3 py-1 md:py-1.5 border-b border-white/5 bg-white/[0.02]">
              <span className="text-[6px] md:text-[8px] font-black text-white/40 md:text-white/60 uppercase tracking-[0.2em] md:tracking-[0.3em] truncate italic text-center">
                {match.competition}
              </span>
            </div>

            {/* Content: Combatants & Time */}
            <div className="p-3 md:p-5 flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4 flex-1">
              <div className="flex md:flex-col items-center gap-2 flex-1 min-w-0">
                <img src={match.homeLogo} className="w-6 h-6 md:w-10 md:h-10 object-contain group-hover:scale-110 transition-transform" alt="" />
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                <span className="text-sm md:text-xl font-black text-kickr italic tabular-nums leading-none tracking-tighter">
                  {new Date(match.matchDate).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false })}
                </span>
                <span className="text-[6px] md:text-[7px] font-mono text-white/30 uppercase tracking-[0.1em] md:tracking-[0.2em] mt-1 md:mt-1.5 font-bold">
                  {new Date(match.matchDate).toLocaleDateString('en-US', { day: '2-digit', month: 'short' }).toUpperCase()}
                </span>
              </div>

              <div className="flex md:flex-col items-center gap-2 flex-1 min-w-0">
                <img src={match.awayLogo} className="w-6 h-6 md:w-10 md:h-10 object-contain group-hover:scale-110 transition-transform" alt="" />
