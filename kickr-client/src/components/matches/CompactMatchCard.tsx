    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between px-2 py-1 border-b border-white/5 bg-white/[0.01] flex-shrink-0">
                <span className="text-[10px] md:text-[8px] font-black text-main/70 md:text-main/80 uppercase tracking-[0.1em] md:tracking-[0.3em] truncate italic">
                    {match.competition}
                </span>
                {!isFinished && (
                    <span className="md:hidden text-[9px] font-mono text-main/40 uppercase font-bold">
            ...
            <div className="p-1.5 md:p-5 flex items-center justify-between gap-1.5 md:gap-4 flex-1">
                <div className="flex items-center justify-center md:flex-col gap-1 md:gap-2 flex-1 min-w-0">
                    ...
                    <span className="hidden md:block text-[10px] font-black text-main/80 uppercase italic truncate w-full text-center">{match.homeTeam}</span>
                </div>
                ...
                <div className="flex flex-col items-center flex-shrink-0">
                    {isFinished ? (
                        <div className="flex flex-col items-center">
                            ...
                            <span className="text-[9px] md:text-[7px] font-mono text-main/60 uppercase tracking-widest font-bold mt-0.5">FT</span>
                        </div>
                    ) : (
                        <div className="flex flex-col items-center">
                            ...
                            <span className="hidden md:block text-[7px] font-mono text-main/70 uppercase tracking-[0.2em] mt-1.5 font-bold">
                                {new Date(match.matchDate).toLocaleDateString('en-US', { day: '2-digit', month: 'short' }).toUpperCase()}
                            </span>
                        </div>
                    )}
                ...
                <div className="flex items-center justify-center md:flex-col gap-1 md:gap-2 flex-1 min-w-0">
                    ...
                    <span className="hidden md:block text-[10px] font-black text-main/80 uppercase italic truncate w-full text-center">{match.awayTeam}</span>
                </div>
            </div>
        </Link>
    );
