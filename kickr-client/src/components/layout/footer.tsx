...
export const Footer = () => {
  return (
    <footer className="bg-[#14181c] border-t border-white/5 py-16 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <div className="flex flex-col items-center md:items-start">
            <Link to="/" className="flex items-center group mb-4">
              <span className="text-xl font-black italic tracking-tighter text-white/90 display-font group-hover:text-kickr transition-colors">KICKR</span>
            </Link>
            <p className="text-white/40 text-[9px] font-black uppercase tracking-[0.3em] leading-relaxed text-center md:text-left">
          ...
          <div className="flex items-center gap-6">
            ...
            />
            <div className="text-[10px] font-black text-white/20 uppercase tracking-widest cursor-default select-none">
              v0.1.0-KICK
            </div>
          </div>
        </div>
...
const FooterLink = ({ label, to }: { label: string; to: string }) => (
  <Link
    to={to}
    className="text-white/40 hover:text-white/80 transition-all duration-300 text-[10px] font-black uppercase tracking-[0.2em] relative group"
  >
    {label}
...
const SocialIcon = ({ icon, href }: { icon: string; href: string }) => (
  <a
    href={href}
    className="text-white/40 hover:text-white transition-colors duration-300 text-xs grayscale hover:grayscale-0"
  >
    {icon}
  </a>
);