      ...
      <div className="bg-black/[0.02] border-b border-white/5 pt-20 pb-6 md:py-24 relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center gap-3 md:gap-12 relative z-10">
          <div className="w-16 h-16 md:w-32 md:h-32 bg-kickr-bg-primary rounded-sm p-3 md:p-6 border border-white/10 shadow-2xl flex-shrink-0">
            <img src={competition.logoUrl} alt={`${competition.name} logo`} className="w-full h-full object-contain filter drop-shadow-xl" />
          </div>
          ...
          <div className={`${!isTournament ? 'lg:col-span-4' : 'w-full'} space-y-6 md:space-y-8`}>
            <header className="flex items-center justify-between border-b border-white/5 pb-3 md:pb-4">
              <div>
                ...
                <p className="text-[7px] md:text-[10px] text-main/40 font-bold uppercase tracking-widest mt-0.5 md:mt-1">Season Timeline</p>
              </div>
                ...
                {totalMatchPages > 1 && (
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      aria-label="Previous match page"
                      className="text-[10px] font-black uppercase text-secondary hover:text-main disabled:opacity-20 transition-all font-mono"
                    >
                      ←
                    </button>
                    <span className="text-[9px] font-mono text-kickr/70 font-black">{matchPage + 1}/{totalMatchPages}</span>
                    <button
                      disabled={matchPage >= totalMatchPages - 1}
                      onClick={() => setMatchPage(p => p + 1)}
                      aria-label="Next match page"
                      className="text-[10px] font-black uppercase text-secondary hover:text-main disabled:opacity-20 transition-all font-mono"
                    >
                      →
                    </button>
            ...
            <div className="space-y-4">
              {isLoadingMatches ? (
                ...
                <div className="py-12 border border-white/5 rounded-sm bg-white/[0.01] text-center">
                  <p className="text-[8px] font-black text-main/40 uppercase tracking-[0.3em] italic">No Records found.</p>
                </div>
              ) : (
...
const CompStat = ({ label, value }: { label: string; value: string | number }) => (
  <div className="text-left">
    <div className="text-[7px] md:text-[9px] font-bold text-muted uppercase tracking-[0.2em] mb-0.5 md:mb-1">{label}</div>
