...
export const UserDiaryPage = () => {
    ...
    return (
        <main className="min-h-screen bg-[#14181c] text-white/40">
            ...
            <div className="max-w-5xl mx-auto px-4 md:px-6 -mt-[180px] md:-mt-[300px] relative z-20 pb-16 md:pb-20">
                <header className="mb-8 md:mb-16">
                    <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-8">
                        <Link to={`/user/${id}`} className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-white/20 hover:text-kickr transition-colors flex items-center gap-1.5 group italic">
                        ...
                        <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-white/10 italic">Match Records</span>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-8 mb-8 md:mb-12">
                        <div className="flex items-center gap-4 md:gap-8">
                            {isOwnProfile ? (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/link:opacity-100 transition-opacity bg-[#14181c]/60 backdrop-blur-sm">
                                            <span className="text-[8px] md:text-[10px] font-black text-white uppercase tracking-widest">Edit</span>
                                        </div>
                                    </Link>
                                </motion.div>
                            ) : (
                            ...
                            <div>
                                <motion.h1
                                    ...
                                    className="text-2xl md:text-5xl font-black text-white italic tracking-tighter uppercase display-font leading-none mb-1"
                                >
                                    {user?.name}<span className="text-kickr/80">'s</span> Match Log
                                </motion.h1>
                                <p className="text-white/10 uppercase tracking-[0.2em] md:tracking-[0.3em] text-[7px] md:text-[9px] font-black italic">
                        ...
                        <div className="flex gap-6 md:gap-10 md:border-l border-white/5 md:pl-10">
                            <div className="flex flex-col items-center md:items-end">
                                <span className="text-base md:text-2xl font-black text-white italic leading-none tracking-tighter tabular-nums">{pageData?.totalElements || 0}</span>
                                <span className="text-[6px] md:text-[8px] uppercase tracking-widest text-white/20 font-black mt-1">LOGS</span>
                            </div>
                            <div className="flex flex-col items-center md:items-end">
                                ...
                                <span className="text-[6px] md:text-[8px] uppercase tracking-widest text-white/20 font-black mt-1">SAVES</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center bg-white/[0.01] border border-white/5 p-2 md:p-4 rounded-sm gap-2 md:gap-4">
                        ...
                        <div className="flex flex-col gap-0 flex-1">
                            <span className="text-[6px] md:text-[8px] uppercase font-black text-white/5 tracking-[0.2em] italic">FIND TEAMS</span>
                            <input
                                type="text"
                                placeholder="..."
                                value={search}
                                onChange={(e) => setSearch(e.target.value)}
                                className="bg-transparent text-[9px] md:text-[11px] font-black text-white/40 placeholder-white/5 outline-none w-full uppercase italic tracking-widest"
                            />
                        </div>
                    </div>
                </header>
                    ...
                    <div className="space-y-8 md:space-y-16">
                        {Object.entries(groupedReviews).map(([monthYear, monthReviews]: [string, any]) => (
                            <section key={monthYear} className="bg-white/[0.01] p-4 md:p-6 rounded-sm border border-white/5">
                                <h2 className="text-[8px] md:text-[10px] font-black text-white/20 uppercase tracking-[0.3em] mb-4 md:mb-6 border-b border-white/5 pb-1.5 italic">
                                    {monthYear}
                                </h2>
                                ...
                                <div className="space-y-3 md:space-y-4">
                                    {monthReviews.map((review: any) => (
                                        <div key={review.id} className="group relative">
                                            <div className="flex gap-3 md:gap-6 p-2 md:p-4 rounded-sm border border-white/5 bg-white/[0.01] hover:bg-white/[0.03] hover:border-kickr/20 transition-all duration-300 items-center">
                                                <div className="w-7 md:w-12 text-center border-r border-white/5 md:pr-6 flex-shrink-0">
                                                    <div className="text-xs md:text-xl font-black text-white italic leading-none tabular-nums">{new Date(review.watchedAt).getDate()}</div>
                                                    <div className="text-[5px] md:text-[8px] font-black uppercase tracking-widest text-white/5 mt-0.5 md:mt-1 italic">
                                                ...
                                                <Link to={`/matches/${review.match.id}`} className="w-16 md:w-32 h-8 md:h-16 bg-white/[0.01] rounded-sm border border-white/5 overflow-hidden flex-shrink-0 flex items-center justify-center gap-1.5 md:gap-3 relative group/poster">
                                                    ...
                                                    <div className="flex flex-col items-center z-10">
                                                        <span className="text-[8px] md:text-xs font-black text-white italic leading-none tabular-nums opacity-60">{review.match.homeScore}</span>
                                                        <div className="w-1.5 md:w-4 h-[1px] bg-white/10 my-0.5 md:my-1"></div>
                                                        <span className="text-[8px] md:text-xs font-black text-white italic leading-none tabular-nums opacity-60">{review.match.awayScore}</span>
                                                    </div>
                                                ...
                                                <div className="flex-1 min-w-0 pr-2">
                                                    <div className="flex flex-col md:flex-row md:items-center gap-0.5 md:gap-3 mb-0.5">
                                                        <Link to={`/matches/${review.match.id}`} className="text-white/80 text-[10px] md:text-base font-black uppercase italic tracking-tighter hover:text-kickr transition-colors leading-none truncate block max-w-[100px] md:max-w-none">
                                                            {review.match.homeTeam} VS {review.match.awayTeam}
                                                        </Link>
                                                        <span className="text-[5px] md:text-[9px] font-black uppercase tracking-widest text-white/5 px-1 md:px-2 py-0.5 rounded-sm border border-white/[0.03] bg-white/[0.01] w-fit italic">
                                                            {review.match.competition}
                                                        </span>
                                                    </div>
                                                    {review.comment && (
                                                        <p className="text-white/10 text-[8px] md:text-xs italic line-clamp-1 group-hover:text-white/20 transition-colors uppercase font-medium">
                                                            {review.comment}
                                                        </p>
                                                    )}
                                                </div>
                        ...
                        {!isLoading && pageData && pageData.totalPages > 1 && (
                            <div className="mt-8 md:mt-16 flex items-center justify-center gap-2 md:gap-4 pb-12">
                                <button
                                    ...
                                    className="px-3 md:px-6 py-2 md:py-3 bg-white/[0.01] border border-white/5 rounded-sm text-[8px] md:text-[10px] font-black uppercase tracking-widest text-white/20 hover:text-white hover:border-kickr/40 disabled:opacity-5 transition-all cursor-pointer italic"
                                >
                                    Prev
                                </button>
                                ...
                                <div className="flex items-center gap-1 md:gap-2">
                                    {[...Array(pageData.totalPages)].map((_, i) => {
                                        if (pageData.totalPages > 5) {
                                            ...
                                            if (i === currentPage - 2 && i !== 0) return <span key={i} className="text-white/5 text-[8px]">...</span>;
                                            if (i === currentPage + 2 && i !== pageData.totalPages - 1) return <span key={i} className="text-white/5 text-[8px]">...</span>;
                                        }
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-7 h-7 md:w-10 md:h-10 rounded-sm text-[8px] md:text-[10px] font-black transition-all cursor-pointer tabular-nums ${currentPage === i
                                                    ? 'bg-kickr text-black'
                                                    : 'bg-white/[0.01] border border-white/5 text-white/10 hover:text-white hover:border-white/10'
                                                    }`}
                                            >
                                                {i + 1}
                                            </button>
